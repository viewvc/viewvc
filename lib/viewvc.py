# -*-python-*-
#
# Copyright (C) 1999-2026 The ViewCVS Group. All Rights Reserved.
#
...
class Request:
    ...
    def run_viewvc(self):
        ...
        if self.rootname == view_roots_magic:
            ...
            if cfg.options.root_as_url_component:
                if path_parts:
                    roottype, rootpath, self.rootname, new_path_parts = locate_root_from_path(
                        cfg, path_parts
                    )
                    if roottype is None:
            ...
            if roottype:
                ...
                try:
                    if roottype == "cvs":
                        ...
                        self.repos = vclib.ccvs.CVSRepository(
                            self.rootname,
                            self.rootpath,
                            self.auth,
                            cfg.utilities,
        ...
        if self.repos and self.view_func is not redirect_pathrev:
            ...
            if self.pathtype is None:
                ...
                # with a fake suffix.
                result = (
                    _strip_suffix(".diff", path_parts, pathrev, vclib.FILE, self.repos, view_diff)
                    or _strip_suffix(
                        ".tar.gz", path_parts, pathrev, vclib.DIR, self.repos, download_tarball
                    )
                    or _strip_suffix(
                        "root.tar.gz", path_parts, pathrev, vclib.DIR, self.repos, download_tarball
                    )
                    or _strip_suffix(
                        self.rootname + "-root.tar.gz",
                        path_parts,
                        pathrev,
                        vclib.DIR,
                        self.repos,
                    ...
                    or _strip_suffix(
                        "root", path_parts, pathrev, vclib.DIR, self.repos, download_tarball
                    )
                    or _strip_suffix(
                        self.rootname + "-root",
                        path_parts,
        ...
        if self.view_func is None:
            ...
            if not self.rootname:
                ...
                elif "revision" in self.query_dict or cfg.options.default_file_view != "log":
                    if cfg.options.default_file_view == "markup" or (
                        self.query_dict.get("content-type", None)
    ...
    def get_form(self, **args):
        ...
        url, params = self.get_link(*(), **args)
        action = self.server.escape(_quote(url, _URL_SAFE_CHARS, "utf-8", "surrogateescape"))
        hidden_values = []
        for name, value in params.items():
            hidden_values.append(
                _item(name=self.server.escape(name), value=self.server.escape(value))
            )
    ...
    def get_link(self, view_func=None, where=None, pathtype=None, params=None):
        ...
        if view_func is view_roots and (
            cfg.options.root_as_url_component or not cfg.general.default_root
        ):
            view_func = None
        ...
        if view_func is view_diff and params.get("r1") is not None and params.get("r2") is not None:
            view_func = None
        ...
        if view_func is view_markup:
            if cfg.options.default_file_view == "markup" and pathtype == vclib.FILE:
                view_func = None
...
def check_freshness(request, mtime=None, etag=None, weak=0):
    ...
    if mtime is not None:
        try:
            ...
            request_mtime = email.utils.mktime_tz(email.utils.parsedate_tz(request_mtime))
        except Exception:
            request_mtime = None
...
def get_writeready_server_file(
    request,
    content_type=None,
    encoding=None,
    content_length=None,
    ...
    if allow_compress and request.gzip_compress_level:
        fp = gzip.GzipFile("", "wb", request.gzip_compress_level, request.server.file())
    else:
        fp = request.server.file()

    if is_text:
        fp = TextIOWrapper_noclose(fp, "utf-8", "surrogateescape", write_through=True)
...
def prep_tags(request, tags):
    ...
    if params:
        url = _quote(url, _URL_SAFE_CHARS, "utf-8", "surrogateescape") + "?" + params + "&pathrev="
    else:
        url = _quote(url, _URL_SAFE_CHARS, "utf-8", "surrogateescape") + "?pathrev="
...
def get_file_view_info(request, where, rev=None, mime_type=None, pathrev=-1):
    ...
    if "markup" in request.cfg.options.allowed_views:
        view_href = request.get_url(
            ...
            where=where,
            pathtype=vclib.FILE,
            params={"revision": rev, "pathrev": pathrev},
            escape=1,
        )
    if "co" in request.cfg.options.allowed_views:
        ...
        if not is_plain_text(mime_type):
            download_text_href = request.get_url(
                ...
                where=where,
                pathtype=vclib.FILE,
                params={"content-type": "text/plain", "revision": rev, "pathrev": pathrev},
                escape=1,
            )
...
_re_rewrite_email = re.compile(r"([-a-zA-Z0-9_.\+]+)@" r"(([-a-zA-Z0-9]+\.)+[A-Za-z]{2,4})")
                ...
                if self.request.roottype == "svn":
                    ...
                    def revision_to_url(rev):
                        return self.request.get_url(
                            view_func=view_revision, params={"revision": rev}, escape=0
                        )

                    lf.add_formatter(_re_rewrite_svnrevref, lf.format_svnrevref, revision_to_url)
...
def calculate_mime_type(request, path_parts, rev):
    ...
    if not path_parts:
        return None, None
    mime_type = None
    if request.roottype == "svn" and (not request.cfg.options.svn_ignore_mimetype):
        try:
            itemprops = request.repos.itemprops(path_parts, rev)
...
def view_cvsgraph(request):
    ...
    fp = request.repos.cvsgraph_popen(
        (
            ...
            "x",
            "-3",
            request.get_url(view_func=view_log, params={}, escape=1),
            "-4",
            request.get_url(view_func=view, params={"revision": None}, escape=1, partial=1),
            "-5",
            request.get_url(
                view_func=view_diff, params={"r1": None, "r2": None}, escape=1, partial=1
            ),
            "-6",
            request.get_url(
                view_func=view_directory,
    ...
    if f1 and f2:
        ...
        while parsing:
            ...
            if line[: len(f1)] == f1:
                ...
                parsing = 0
            elif line.find("not found") != -1 or line.find("illegal option") != -1:
                flag = _RCSDIFF_ERROR
                parsing = 0
...
def _get_diff_path_parts(request, query_key, rev, base_rev):
    ...
    if query_key in request.query_dict:
        ...
        try:
            ...
            raise ViewVCException(
                "Invalid path(s) or revision(s) passed to diff", "400 Bad Request"
            )
        except vclib.ItemNotFound:
            raise ViewVCException(
                "Invalid path(s) or revision(s) passed to diff", "400 Bad Request"
...
class DiffDescription:
    ...
    def get_prop_diff(self, left, right):
        ...
        for name in self._uniq(list(left.properties.keys()) + list(right.properties.keys())):
            ...
            if is_undisplayable(val_left) or is_undisplayable(val_right):
                ...
                continue
            self._get_diff(left, right, self._prop_lines, self._prop_fp, diff_options, name)
    ...
    def _line_idiff_sidebyside(self, lines_left, lines_right, diff_options):
        return idiff.sidebyside(lines_left, lines_right, diff_options.get("context", 5))

    def _line_idiff_unified(self, lines_left, lines_right, diff_options):
        return idiff.unified(lines_left, lines_right, diff_options.get("context", 2))
    ...
    def _fp_vclib_raw(self, left, right, fp, propname):
        ...
        if flag is not None:
            ...
            mpw = MarkupPipeWrapper(fp, self.request.server.escape(headers), None, 1)
...
def locate_root_from_path(cfg, path_parts):
    ...
    rootname, path_in_parent, remain = find_root_in_parents(cfg, path_parts, "svn")
    if path_in_parent:
