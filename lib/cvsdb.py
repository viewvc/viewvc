...
class CheckinDatabase:
  ...
  def Connect(self):
    self.db = dbi.connect(self._host, self._port, self._user,
                          self._passwd, self._database)
  ...
  def SQLQueryListString(self, field, query_entry_list):
    ...
    for query_entry in query_entry_list:
      ...
      sqlList.append("%s%s%s"
                     % (field, match, self.db.literal(data).decode('utf-8')))
  ...
  def CreateSQLQueryString(self, query, detect_leftover=0):
    ...
    if len(query.repository_list):
      tableList.append(("repositories",
                        "(%s.repositoryid=repositories.id)" % (commits_table)))
