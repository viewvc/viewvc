    ...
    if match:
        ...
        if include_path:
            return '#%sinclude%s<a href="%s">"%s"</a>' % (
                match.group(1),
                match.group(2),
                include_url.replace("/WHERE/", include_path),
                incfile,
            )
...
class HTMLBlameSource:
    ...
    def __init__(self, repos, path_parts, diff_url, include_url, opt_rev=None):
        ...
        self.annotation, self.revision = self.repos.annotate(path_parts, opt_rev, True)
    ...
    def __getitem__(self, idx):
        ...
        thisline = link_includes(
            sapi.escape(item.text), self.repos, self.path_parts, self.include_url
        )
        return _item(
            text=thisline,
            line_number=item.line_number,
            rev=item.rev,
            prev_rev=item.prev_rev,
            diff_url=diff_url,
            date=item.date,
            author=item.author,
        )
...
def make_html(root, rcs_path):
    ...
    for line_data in bs:
        ...
        sys.stdout.write(
            '<tr id="l%d" style="background-color: #%s; '
