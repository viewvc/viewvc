...
class InfoSink(MatchingSink):
  ...
  def define_revision(self, revision, date, author, state, branches, next):
    ...
    tag = self.find_tag
    rev = Revision(self._to_str(revision), date, self._to_str(author),
                   state == "dead")
...
class TreeSink(rcsparse.Sink):
  ...
  def define_revision(self, revision, date, author, state, branches, next):
    ...
    self.revs[self._to_str(revision)] = \
        Revision(self._to_str(revision), date, self._to_str(author),
                 state == "dead")
